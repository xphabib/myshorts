...
output_directory = "../short_maker/videos"

files = Dir.glob(File.join(output_directory, '*'))
file_count = files.count

times_arr.each_with_index do |time, index|
    system("ffmpeg -i input.mp4 -ss #{time[:start]} -to #{time[:end]} -c:v libx264 -c:a aac #{output_directory}/video_#{file_count+index+1}.mp4")
end
